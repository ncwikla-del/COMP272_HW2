...
public class BinaryTree {
    ...
    private void replaceValueHelper(Node node, int oldVal, int newVal) {
       // When the node is null, do not continue
        if (node == null) {
            return;
        }
       // If a node's data is same as old value, instead make it new value
        if (node.data == oldVal) {
            node.data = newVal;
        }
    ...
    private int findMinHelper(Node node) {
       // If node is empty return the max value of it.
        if (node == null) {
            return Integer.MAX_VALUE;
        // Otherwise, if not empty, use Math.min to find the min between left and right
        } else {
    ...
    private int nodesGTHelper(Node node, int val) {
        // If node/tree is empty return 0
        if (node == null) {
            return 0;
        }
        // Initialize count for the current node
        int count = 0;
        // Check if current nodes "data" is greater than value
        if (node.data > val) {
            count = 1; // Increments count if it is greater
        }
        //Recursively go through tree nodes on left and right subtrees and add them to the count
        count += nodesGTHelper(node.left, val);
        count += nodesGTHelper(node.right, val);
        // Return the count of nodes greater than val
        return count;
    }
    ...
    private int[] averageHelper(Node n) {
        // If the tree is empty sum and count are both returned as 0.
        if (n == null) {
            return new int[]{0,0};
        }
        // Initialize the nodes that are a part of left of the tree to get the sum and count.
        int [] left = averageHelper(n.left);
        // Initialize the nodes that are part of the right of the tree to get the sum and count.
        int [] right = averageHelper(n.right);
        // Add left and right sum, including the node's data to get the total sum.
        int sum = left[0] + right [0] + n.data;
        // Get the count by again adding left + right but adding 1 for current node.
        int count = left[1] + right[1] + 1;
        // Return sum and count as an array to be calculated in the 'average' method.
